...
function StatusBadge({ status }) {
  ...
  return (
    <span style={{ padding: '3px 10px', borderRadius: '99px', fontSize: '12px', fontWeight: '600', background: s.bg, color: s.color, border: `1px solid ${s.border}`, whiteSpace: 'nowrap' }}>{s.label}</span>
  );
}
...
function AdminDashboard() {
  ...
  const changeStatus = async (booking, newStatus) => {
    ...
    setUpdating(id);
    try {
      await axios.patch(`${API}/api/admin/bookings/${id}`, { status: newStatus });
      setBookings(bookings.map(b => (b.id || b.booking_id) === id ? { ...b, status: newStatus } : b));
    } catch (err) {
      console.error(err);
  ...
  const filtered = bookings.filter(b => {
    ...
    const matchSearch = !search || b.hotel_name?.toLowerCase().includes(search.toLowerCase()) || b.user_name?.toLowerCase().includes(search.toLowerCase());
    return matchStatus && matchSearch;
  });

  const summary = STATUSES.reduce((acc, s) => { acc[s] = bookings.filter(b => b.status?.toUpperCase() === s).length; return acc; }, {});
    ...
    <>
      ...
      <div className="admin-wrap">
        <h2 style={{ fontSize: '22px', fontWeight: '700', color: '#111', margin: '0 0 20px' }}>
          ...
          <span style={{ marginLeft: '8px', fontSize: '14px', color: '#9ca3af', fontWeight: '400' }}>({bookings.length} รายการทั้งหมด)</span>
        </h2>
        ...
        <div className="summary-grid">
          {STATUSES.map(s => {
            ...
            return (
              <div key={s} className="summary-card" style={{ background: cfg.bg, borderColor: cfg.border }} onClick={() => setFilterStatus(filterStatus === s ? 'ALL' : s)}>
        ...
        <div className="filter-bar">
          <input className="search-inp" placeholder="ค้นหาชื่อโรงแรม / ผู้จอง..." value={search} onChange={e => setSearch(e.target.value)} />
        ...
        <div style={{ border: '1px solid #e5e7eb', borderRadius: '12px', overflow: 'hidden' }}>
          {loading ? (
            ...
            <table className="admin-table">
              <thead>
                <tr><th>โรงแรม</th><th>ผู้จอง</th><th>เช็คอิน</th><th>เช็คเอาท์</th><th>สถานะ</th><th>เปลี่ยนสถานะ</th></tr>
              </thead>
              <tbody>
                {filtered.map(b => {
                  ...
                  return (
                    <tr key={id}>
                      ...
                      <td>
                        <select className="status-select" value={b.status?.toUpperCase() || 'PENDING'} disabled={updating === id} onChange={e => changeStatus(b, e.target.value)}>
                          {STATUSES.map(s => <option key={s} value={s}>{STATUS_CONFIG[s].label}</option>)}
                        </select>
                      </td>
                    </tr>
